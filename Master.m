classdef Master < handle
    properties
        ...
        depth_weight = 0.5;     % 远见值：AI对空间的利用效率提高，即在相同的空格中走出更多的步数。
        node_weight = 0.05;     % 后备值：AI的每一步走棋带来更多的后备选择，减少意外发生。
        control_weight = 1.2;   % 控制值：AI的进攻性增强，尽可能减少玩家的选择。
        defense_weight = 0.45;  % 防御值：AI的反侦察意识增强，减少被玩家封堵的可能。
    end
                ...
                if sum(obj.board,"all") < 20
                    ...
                    if sum(choice_list) == 0
                        ...
                        if size(to_be_selected,1) == 1
                            ...
                            for k = 1:length(size(to_be_selected,1))
                            ...
                            value = maxdepth_ai * obj.depth_weight + maxnode_ai * obj.node_weight + summary * obj.control_weight - total * obj.defense_weight;   % 四部分分别为：最大深度，子节点数量，限制对手数量，被对手限制数量
