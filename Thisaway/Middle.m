classdef Middle < handle
    ...
    methods
        ...
        function createBoard(obj)
            ...
            xlim([0.5, 7.5]);
            ylim([0.5, 7.5]);
            title(sprintf('Thisaway(Middle) 抢4赛 (第%d局)', obj.roundNumber));
        end
            ...
            if ~isempty(possibleMoves)
                ...
                if sum(choice_list) == 0
                    ...
                    for i = 1:length(choice)
                        ...
                        to_be_selected(i) = size(potentialmove,1);
        ...
        function valid = isValidMove(obj, from, to)
            % 检查移动是否符合"马走日"规则 (无蹩马腿规则)
            dx = abs(to(2) - from(2));
            dy = abs(to(1) - from(1));
            
            % 马走日规则：一个方向走1格，垂直方向走2格
